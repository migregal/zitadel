...
func RegisterRoutes(router chi.Router, s {{.Name}}) {
	router.Use(s.AuthInterceptor().Handler)

	{{ range $service := .Services}}
	{{ range $methodName, $method := .Methods}}
	{{ range $handler := .Handlers}}
	router.HandleFunc("{{$handler.Method}} {{$service.Prefix}}{{$method.Path}}{{$handler.PathSuffix}}", {{if $handler.HandlerType}}{{$handler.HandlerType}}(s, {{end}}s.{{$handler.Name}}{{$methodName}}(){{if $handler.HandlerType}}){{end}})
	{{- if $method.HasDarkMode }}
	router.HandleFunc("{{$handler.Method}} {{$service.Prefix}}{{$method.Path}}/dark{{$handler.PathSuffix}}", {{if $handler.HandlerType}}{{$handler.HandlerType}}(s, {{end}}s.{{$handler.Name}}{{$methodName}}Dark(){{if $handler.HandlerType}}){{end}})
	{{- end }}
	{{- end }}
	{{- end }}
	{{- end }}
